...
export default function KadanesAlgorithm() {
  ...
  const getPointerLabel = (index) => {
    ...
    if (index === subarrayStart && subarrayStart !== subarrayEnd) labels.push({ text: "S", color: "text-blue-400 bg-blue-500/20 border border-blue-500/30" });
    if (index === subarrayEnd && subarrayStart !== subarrayEnd) labels.push({ text: "E", color: "text-blue-400 bg-blue-500/20 border border-blue-500/30" });
    return labels;
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-300 p-6 md:p-10 selection:bg-blue-500/30">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-2">Array (comma-separated, negatives allowed)</label>
            <div className="flex gap-3">
              <input
                type="text"
                value={arrayInput}
                onChange={(e) => setArrayInput(e.target.value)}
                className="flex-1 bg-slate-950 border border-slate-800 focus:border-amber-500 focus:ring-1 focus:ring-amber-500/50 text-slate-200 p-3 rounded-xl outline-none transition-all placeholder:text-slate-600"
                placeholder="e.g. -2, 1, -3, 4, -1, 2, 1, -5, 4"
              />
              <button onClick={parseArrayInput} className="px-5 py-3 bg-amber-600/20 hover:bg-amber-600/30 text-amber-400 border border-amber-500/30 rounded-xl font-semibold transition-all">
                Set Array
              </button>
            </div>
          </div>
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl flex flex-wrap gap-6 items-center justify-between">
          ...
          <div className="flex flex-col gap-2 w-full sm:w-auto">
              ...
              <input type="range" min={200} max={2000} step={100} value={speed} onChange={(e) => setSpeed(Number(e.target.value))} className="w-full sm:w-48 accent-amber-500 h-2 bg-slate-800 rounded-lg appearance-none cursor-pointer"/>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl min-h-[300px] flex flex-col justify-center relative overflow-hidden">
          ...
          {currentFrame && currentFrame.maxStart !== undefined && (
            <div className="mt-8 flex justify-center w-full">
              <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl px-6 py-3 text-sm flex items-center gap-3">
          ...
          <div className="lg:col-span-2 space-y-6 flex flex-col">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 text-center shadow-lg">
                ...
                <div className="text-2xl font-bold text-slate-200">{currentFrame ? currentFrame.currentIndex : "—"}</div>
              </div>
              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 text-center shadow-lg">
                <div className="text-[10px] text-blue-400 font-semibold uppercase tracking-wider mb-1">Current Sum</div>
              ...
              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 text-center shadow-lg flex flex-col justify-center">
                <div className="text-[10px] text-purple-400 font-semibold uppercase tracking-wider mb-1">Decision</div>
            ...
            <div className={`flex-1 rounded-3xl p-6 sm:p-8 border shadow-xl transition-all duration-300 ${
                currentFrame?.type === "completed" ? "bg-emerald-500/10 border-emerald-500/30"
                : currentFrame?.type === "new_max" ? "bg-amber-500/10 border-amber-500/30"
                : currentFrame?.type === "restart" ? "bg-rose-500/10 border-rose-500/30"
            ...
            <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-xl">
                ...
                <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-slate-300 text-sm font-medium">Current Index</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="w-5 h-5 bg-blue-500/20 border-2 border-blue-400 rounded"></div>
                        <span className="text-slate-300 text-sm font-medium">Current Subarray</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="w-5 h-5 bg-emerald-500/10 border-2 border-emerald-400/50 rounded ring-2 ring-emerald-400/50"></div>
                        <span className="text-slate-300 text-sm font-medium">Best Subarray</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="w-5 h-5 bg-emerald-500/20 border-2 border-emerald-400 rounded"></div>
                        <span className="text-slate-300 text-sm font-medium">Final Answer</span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-xl">
                ...
                <div className="text-slate-300 space-y-3 text-sm leading-relaxed">
                    <div className="p-3 rounded-xl bg-slate-800/50 border border-slate-700/50">
                        <strong className="text-blue-400 block mb-0.5">Extend or Restart</strong>
