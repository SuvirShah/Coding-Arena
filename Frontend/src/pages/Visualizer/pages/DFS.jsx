...
export default function DFS() {
  ...
  const getNodeStyle = (node) => {
    ...
    if (node.isVisited) return "bg-purple-300 animate-pulse"; // DFS uses purple for distinction
    return "bg-slate-100 border border-slate-200 hover:bg-slate-200";
  };

  return (
    <div 
      className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-rose-100 p-8"
      onMouseUp={handleMouseUp}
      onMouseLeave={handleMouseUp}
    >
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <Link to="/visualizer" className="inline-flex items-center text-purple-600 hover:text-purple-800 mb-4 font-medium transition-colors">
          <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
          Back to Home
        </Link>
        <h1 className="text-4xl font-bold text-gray-800 mb-2">🕳️ Depth-First Search (DFS) Visualizer</h1>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 mb-6 border border-white/40">
          <div className="flex flex-wrap gap-4 items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <input type="range" min={10} max={500} step={10} value={speed} onChange={(e) => setSpeed(Number(e.target.value))} className="w-28 accent-purple-500" dir="rtl" />
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 mb-6 border border-white/40">
          <div className="flex flex-wrap gap-6 justify-center">
            ...
            <div className="flex items-center gap-2"><div className="w-6 h-6 bg-purple-300 rounded-md"></div><span className="text-sm font-medium text-gray-600">Visited Node</span></div>
            <div className="flex items-center gap-2"><div className="w-6 h-6 bg-yellow-400 rounded-md shadow-md ring-1 ring-yellow-500"></div><span className="text-sm font-medium text-gray-600">Final Path</span></div>
          </div>
        </div>
